...
const FinancialDNA: React.FC<Props> = ({ transactions, categoryBreakdown }) => {
  let data: { name: string; value: number }[];
  let totalSpent: number;

  if (categoryBreakdown && categoryBreakdown.length > 0) {
    // Use backend category_breakdown directly
    data = categoryBreakdown.map(c => ({ name: c.category, value: c.amount }));
    totalSpent = data.reduce((acc, curr) => acc + curr.value, 0);
  } else {
  ...
  return (
    <div className="h-full bg-white rounded-2xl border border-airbnb-line shadow-card p-8 flex flex-col">
      ...
      {data.length === 0 ? (
        <div className="flex-1 flex items-center justify-center text-airbnb-gray text-sm">
          No spending data available. Seed transactions in Settings.
        </div>
      ) : (
